...
function ExportPanel({ project }: { project: ProjectDetail }) {
  ...
  const download = () => {
    // Simple portable export: a shell script that recreates the project.
    const script = [
      "#!/bin/sh",
      `# ${project.name} — generated by Builder. Run: sh setup.sh && cd ${project.projectName} && npx expo start`,
      `mkdir -p ${project.projectName}`,
      ...project.files.flatMap((f) => {
        const dir = f.path.includes("/") ? f.path.slice(0, f.path.lastIndexOf("/")) : "";
        const mk = dir ? [`mkdir -p ${project.projectName}/${dir}`] : [];
        return [...mk, `cat > ${project.projectName}/${f.path} << 'BUILDER_EOF'`, f.contents, "BUILDER_EOF"];
      }),
      `echo "Done. cd ${project.projectName} && npx expo install && npx expo start"`,
    ].join("\n");
    const blob = new Blob([script], { type: "text/x-sh" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `${project.projectName}-setup.sh`;
    a.click();
  ...
  return (
    <div className="builder-export">
      ...
      <ol>
        <li>Download the project setup script.</li>
        <li>
          <code>sh {project.projectName}-setup.sh</code>
        </li>
        <li>
