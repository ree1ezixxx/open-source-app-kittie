...
/* Metro config: disable Watchman. Generated workspaces live under a large
   home-dir tree where `watchman watch-project` resolves a huge watch root and
   hangs indefinitely, stalling the bundler. Node's built-in file watcher
   crawls only the project dir and boots reliably for previews. */
function metroConfig(): string {
...
export function generateExpoProject(b: AppBlueprint): { slug: string; files: GeneratedFile[] } {
  ...
  const files: GeneratedFile[] = [
    ...
    { path: "metro.config.js", contents: metroConfig() },
