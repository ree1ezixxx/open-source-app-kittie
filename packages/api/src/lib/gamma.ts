...
/**
 * Cache-through JSON generation: return the stored output for
 * (kind, subjectId, hash(input)) if present, else generate once and store.
 * Mirrors the Gemini `cachedGenerate` contract but records `GAMMA_MODEL`.
 */
export async function cachedGammaJson<T>(
  kind: string,
  subjectId: string,
  input: string,
  prompt: string,
  ...
  const output = await gammaJsonRaw(prompt, opts);
