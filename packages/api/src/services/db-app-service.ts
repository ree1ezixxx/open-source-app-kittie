...
// makes the API pick up the worker's new snapshot day on its own within the
// window — without it, Explore would pin yesterday's day until an API restart,
// silently re-introducing staleness. `invalidateAppReadCaches()` still force-clears
// it for the in-process manual/CLI path.
